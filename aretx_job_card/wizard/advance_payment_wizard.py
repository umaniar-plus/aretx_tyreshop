...
class advance_payment_wizard(models.TransientModel):
    _name = 'job.advance.payment.wizard'
    ...
    def payment_process(self):
        ...
        if self._context.get('active_model') == 'job.card' :#or self._context.get('active_model') == 'job.card':
            for obj in self:
                ...
                if job_obj:
                    job_id = job_obj[0]
#                     if job_id.amount_total < job_id.total_advance + obj.amt:
#                         raise UserError("Advance Amount Should not be greater than Total Amount.")
                ...
                if job_obj and job_id.id:
                    self._cr.execute('insert into job_sale_account_move_rel(job_id, move_id) values (%s,%s)', (
                    ...
                    self.env['job.card'].write({'total_advance': sum})
            ...
            for obj in self:
                ...
                self._cr.execute(
                    'insert into job_sale_account_move_rel(job_id,move_id) values (%s,%s)',

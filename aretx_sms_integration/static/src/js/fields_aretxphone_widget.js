odoo.define('aretx_sms.fields', function (require) {
"use strict";
    ...
    _renderReadonly: function () {
        ...
        if (this.enableSMS && this.value) {
            var $composerButton = $('<a>', {
                title: _t('Send SMS Text Message'),
                href: '',
                class: 'ml-3 d-inline-flex align-items-center o_field_phone_aretx_sms',
                html: $('<small>', {class: 'font-weight-bold ml-1', html: 'Aretx SMS'}),
            });
            $composerButton.prepend($('<i>', {class: 'fa fa-mobile'}));
            $composerButton.on('click', this._onClickAretxSMS.bind(this));
